...
ITERATION 2 - SEQUENTIAL RULE CHAINING:
✅ NSM: Extract symbolic rules + dependencies
✅ SDPM: Chain programs based on rule order
✅ Sequential execution: rule1 → rule2 → rule3
   - Build transformation pipeline
   - Each rule transforms previous output
   - Final output = chain result

PIPELINE FLOW:
training_examples → NSM(extract+order) → SDPM(chain) → execute_sequence → solution

FOCUS: Sequential rule application instead of parallel strategies.
...
class TurboOrcaV9:
    ...
    def solve_task(self, train_pairs: List[Tuple[np.ndarray, np.ndarray]],
        ...
        # ITERATION 2: SEQUENTIAL CHAINING - apply programs in sequence
        best_solution = test_input.copy()
        best_score = 0.0

        if nsm_programs:
            # Chain programs: output of one → input of next
            current = test_input.copy()
            for prog in nsm_programs:
                try:
                    current = prog(current)
                    score = self.code_x3._score_candidate(current, train_pairs)
                    if score > best_score:
                        best_solution = current.copy()
                        best_score = score
                except:
                    break  # Stop chain if any step fails
