...
function generateSynthesis(archetypal, contextual, psychological, practical) {
  ...
  const primaryKeywords = (archetypal.keywords && archetypal.keywords.length > 0)
    ? archetypal.keywords.slice(0, 3).join(', ')
  ...
  coreSynthesis += `${
    contextual.position_significance
      ? contextual.position_significance.split('.')[0]
      : 'This card adds crucial context to your reading'
  }. The ${contextual.energy_quality || 'present'} nature of this card's appearance suggests ${
    contextual.energy_quality && contextual.energy_quality.includes('flowing')
      ? 'you\'re ready to embody this fully'
      : 'there\'s inner work to do before external manifestation'
  }.`;

  // EXPANDED integration - linking psychological and practical
  let integrationSynthesis = `To integrate this card's medicine: ${
    psychological.integration_path
      ? psychological.integration_path.split('.')[0]
      : 'begin by acknowledging this energy exists within you'
  }. `;
  integrationSynthesis += `${
    practical.practical_advice
      ? practical.practical_advice.split('.')[0]
  ...
  let deeperInsight = psychological.shadow_work
    ? psychological.shadow_work.split('.').slice(0, 2).join('. ') + '. '
    : 'This card invites deep self-reflection. ';
  ...
  const firstThreeActions = (practical.action_steps && Array.isArray(practical.action_steps))
    ? practical.action_steps.slice(0, 3)
